...
<%
    ...
    if (criteria != null) {
        for (String searchField : searchFields) {
      	...
      	if (selectedFields!=null&&selectedFields.indexOf("Nickname")!=-1)
      	{
			...
			for (ClientSession csession : SessionManager.getInstance().getSessions()) {
				
				String addr = csession.getAddress().toBareJID();
				...
				{		
					...
					if (item!=null)
					{
	                    ...
	                    try {
	                        ...
	                        nick=doc.getRootElement().getText();
	                    } catch (Exception e) {
	                        nick=xml.replace("<nick xmlns=\"http://jabber.org/protocol/nick\">", "").replace("</nick>", "");
	                    }
						
						if (nick.toLowerCase().contains(criteria.toLowerCase()))
						{						
							boolean found=false;
							...
							for (User user : users) {
		                        ...
		                        if (user != null&&jid.equalsIgnoreCase(addr)) {
		                            found=true;
		                            break;
		                        }
		                    }
							...
							{	
								try {
									users.add(userManager.getUser(addr.substring(0,addr.indexOf("@"))));
									Log.info("USER "+addr.substring(0,addr.indexOf("@"))+" über Rufnamen gefunden!");
								} catch (UserNotFoundException e) {
									Log.error("USER "+addr.substring(0,addr.indexOf("@"))+" nicht gefunden!");
								}
							}
						}
					}
...
<form name="f" action="advance-user-search.jsp">
    ...
    <div class="jive-contentBox">
        ...
        <tr class="c1">
            <td width="1%" nowrap=""><fmt:message key="advance.user.search.nickname" />:</td>
            <td class="c2">
                <input checked="" name="Nickname" checked type="checkbox">
            </td>
        </tr>
...
<thead>
    <tr>
        ...
        <th nowrap><fmt:message key="advance.user.search.nickname" /></th>
...
<tbody>
    ...
    <% if (users.isEmpty()) { %>
    <tr>
        <td align="center" colspan="9"><fmt:message key="advance.user.search.no_users" /></td>
    </tr>
    ...
    <tr class="jive-<%= (((i%2)==0) ? "even" : "odd") %>">
       ...
       <td width="33">
         ...
         <td width="33">
           <%= SearchPlugin.getRufnameFromUser(user) %> &nbsp;
         </td>
       <td width="15%">
